...
namespace kiwi { namespace engine {
    ...
    {
        Factory::add<MeterTilde>("meter~", &MeterTilde::create);
    }
    
    std::unique_ptr<Object> MeterTilde::create(model::Object const& model, Patcher & patcher)
    {
        return std::make_unique<MeterTilde>(model, patcher);
    }
    
    MeterTilde::MeterTilde(model::Object const& model, Patcher& patcher):
