...
namespace kiwi { namespace engine {
    ...
    {
        Factory::add<OscTilde>("osc~", &OscTilde::create);
    }
    
    std::unique_ptr<Object> OscTilde::create(model::Object const& model, Patcher & patcher)
    {
        return std::make_unique<OscTilde>(model, patcher);
    }
    
    OscTilde::OscTilde(model::Object const& model, Patcher& patcher):
    ...
    void OscTilde::receive(size_t index, std::vector<tool::Atom> const& args)
    {
        ...
        else if(index == 1 && args[0].isNumber())
        {
            ...
            {
                warning("osc~ inlet 2 doesn't understand [" + args[0].getString() + "]");
            }
        }
    }
