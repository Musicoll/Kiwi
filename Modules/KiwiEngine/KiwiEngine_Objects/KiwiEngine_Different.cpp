...
namespace kiwi { namespace engine {
    ...
    {
        return std::make_unique<Different>(model, patcher);
    }
    
    Different::Different(model::Object const& model, Patcher& patcher):
    Operator(model, patcher)
    {
    }
    
    double Different::compute(double lhs, double rhs) const
    {
        return lhs != rhs ? 1 : 0;
    }
