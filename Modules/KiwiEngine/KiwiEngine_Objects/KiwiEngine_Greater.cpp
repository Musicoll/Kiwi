...
namespace kiwi { namespace engine {
    ...
    {
        return std::make_unique<Greater>(model, patcher);
    }
    
    Greater::Greater(model::Object const& model, Patcher& patcher):
    Operator(model, patcher)
    {
    }
    
    double Greater::compute(double lhs, double rhs) const
    {
        return lhs > rhs ? 1 : 0;
    }
