...
namespace kiwi
{
    namespace model
    {
        ...
        std::unique_ptr<model::Object> ObjectFactory::create(std::string const& name, std::vector<Atom> const& args)
        {
            ...
            if(it != creators.end())
            {
                const auto model_ctor = it->second.model_ctor_fn;
                return std::unique_ptr<model::Object>(model_ctor(args));
            }
