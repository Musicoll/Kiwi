...
namespace kiwi
{
    ...
    public: // methods
        ...
        Task(MeterTildeView & meter, float peak_value):
        m_meter(meter),
        m_peak_value(peak_value)
        {
        }
        ...
        {
            float peak_db = 20 * log10(m_peak_value);
            m_meter.computeActiveLed(peak_db);
            m_meter.repaint();
        }
        
    private: // members
        
        MeterTildeView &    m_meter;
        float               m_peak_value;
    };
    ...
    {
        std::shared_ptr<Task> task(new Task(*this, new_peak));
        m_tasks.push(task);
